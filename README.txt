...
Frontend - Bootstrap 5 (шаблон Bootsrap Dash), JavaScript
БД MySQL: User, Image, Post, Wallet, Comments, WbPost, Payouts, Payouts_bank, Balance, Rathing
Механика работы:
1. После одобрения администратором КТ автоматически загружается на ВБ по апи в один кабинет. Брэнд не создается - все заливается в один брэнд.
2. Файлы должны записываться по ..website/static/uploads. А урлы картинок в таблицу Images с ../uploads
Формат артикулов - user_id-post_id: 1-313, 17-580
3. Таблица Balance балансы по дням для пользователя (это и есть суммарный баланс).
Далее общий доход это сумма балансов пользователя по дням и вычетается все выводы (Payouts).
######################
...
Задачи:
- сделать деплой проекта на бегет

(после деплоя сделать)
- /balance надо чтобы, включалась не по кнопке, а по времени в 00:00
- /payouts при выводе подтверждение по почте делать - добавить в таблицу - было ли подтверждение
- /mail после первой продажи отправлять поздравления с первой продажей
- /mail после одобрения высылать письмо, что 50 руб. зачислены вам
- /admin добавить круговую диаграмму или график столбчатый на главную страницу
- superadmin Если вызывать еще раз добавление, то текст поверх создается. ??
- superadmin Получается добавить картинку одну когда КТ уже создана с текстом. ??

---ХРОНОЛОГИЯ---
8.09
- /rathing изменить на столбчатую диаграмму график
- добавить процент 0,05 как доход от валовой суммы
- добавить уже существующую оплату фикс за картинку - 50 руб.
15.08
- /superadmin при архивировании не появляется задача для администратора в супер-админке
- добавить холд для вывода 1 день, который потом можно будет изменить на 2 недели
- сделать главный лэндинг красивым и законченным с добавленной графикой
- поправил вывод суммы КТ за все время в рейтинге
- добавил вывод в настройках в дизайне
12.08
- в userbalance таблице - столбец payoutsbalance лишний - возможно можно убрать
- /payouts при выводе суммы не происходит обнуления
- добавил вывод в dashbord-info разницы с прошлой неделей
- /rating добавил график вывода баланса
9.08
- /superadmin добавил вывод "заархивировано" и инструкцию к нему
- добавить в Balance что если user_id не определен то не делать запись
- добавил изменения в модель UserBalance
- добавил округление до 0 если баланс отрицательный и убрал доли .00
- исправил запись day_balance
8.08
- реализация выплат
6.08
- /payouts создание механики выпла
- создана таблица UserBalance
4.08
- /rating сравнение с предыдущей неделей и изменение в int рейтинга (-9) и также с КТ
- /utils сделать утилиту по записи данных для каждого пользователя ежедневно в БД
- /models создать модель для КТ-артикулов пользователя
- правка выпадающего меню settings
- сделать таблицу учета выплат и накопления баланса
- /statistics считать накопительным итогом количество продаж всех КТ автора (не заказов) по дням в таблице
- добавить в Balance функцию для записи дневных доходов для ВСЕХ пользователе по ВСЕМ карточкам
3.08
- заново сделать миграцию на db4 (тк что то ошибка выходит в wbpost) в дате
- /views перенести роут rating в rating
- /rating добавил запись артикула товара в таблицу WbPost
22.07
...
21.07
- /comments разнести комментарии системные и не системные, в самый верх надо вынести правки
- на удаленном сервере не записывается в БД пользователь
...
17.07
- подключился к БД удаленной с приложения flask
...
- /comments сейчас всегда пишется Редактор: что не правильно
